...
class Tournament:
    ...
    def championship(self):
        ...
        # display number of part played
        log.info(f"{'='*20} Number of games {'='*20}")
        log.info(f"{self.N_GAMES} games played\n")

        log.info(f"{'='*20} Final score {'='*20}")
        log.info(f"{self.STRAT[0]} black : {scores[0]}")
        log.info(f"{self.STRAT[1]} white : {scores[1]}\n")

        # display winrate percentage
        winrate = scores[0] / self.N_GAMES
        log.info(f"{'='*20} Winrate {'='*20}")
        log.info(f"{self.STRAT[0]} black : {winrate*100:.2f}%")
        log.info(f"{self.STRAT[1]} white : {(1-winrate)*100:.2f}%\n")

        # display end of tournament
        log.info(f"{'='*20} End of tournament {'='*20}")
        # print(scores)
